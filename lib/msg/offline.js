...
exports.pushMessage = function(message, touser, poster, option, callback) {
    ...
    mongoConnect.connect(function(MongoConn) {
        ...
        var pushCache = MongoConn.db(conf.mongodb.mg1.dbname).collection('PushCache');d
        ...
        pushCache.find({
            ...
            for (i = 0, len = toUserArray.length; i < len; i++) {
                counts.push(temp[toUserArray[i]] || 1);
            }
            ...
            MongoConn.db(conf.mongodb.mg1.dbname).collection('PushStack').insert(StackObj, function() {
